...
% Conjugation Rates
Rates.Conjugation_Donors = scaling*4;%Initial.Total_Cell_Population; % Plasmid Conjugation Rate 1.2*10e-10
Rates.Conjugation_Transconjugants = scaling*1e-1*Rates.Conjugation_Donors;%Initial.Total_Cell_Population; % Plasmid Conjugation Rate 1.2*10e-10

% Immigration (Birth) of cells rates
Rates.Immigation = scaling*[4e1; 0];

% Flushing rates
% Rates.Cell_flushing = 1/(2*Mean_Cell_cycle*Initial.Total_Cell_Population); % Cell Death Rate
Rates.F = scaling*[16 12 16]; % In ODE model, these elements represents rd/Kd rr/Kr rt/Kt respectively (subindex d for donors, r for Recipient, and t for transconjugants)
Rates.D_Flushing = scaling*1e1/(1+Initial.D_Cell_Population)*[ 1 1]; % In ODE model, these elements represents Bdd,Bdr Bdt
Rates.R_Flushing = scaling*1e1/(1+Initial.R_Cell_Population)*[.5 1]; % In ODE model, these elements represents Brd,Brr Brt
